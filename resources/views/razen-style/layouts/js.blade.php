...
<script>
    const settings = new Settings({
        @if (Auth::guard('razen_style')->check())
            attributes: {
                color: "{{Auth::guard('razen_style')->user()->color_layout?Auth::guard('razen_style')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('razen_style')->user()->nav_color?Auth::guard('razen_style')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('razen_style')->user()->behaviour?Auth::guard('razen_style')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('razen_style')->user()->layout?Auth::guard('razen_style')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('razen_style')->user()->radius?Auth::guard('razen_style')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('razen_style')->user()->placement?Auth::guard('razen_style')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('razen-style.admin.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
